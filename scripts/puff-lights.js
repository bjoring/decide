...
.describe("escape-window", "escape window before puff duration (in ms)")
    ...
    .describe("prob_stim", "probability of presenting stimulus every second")
    .default({"escape-window": 2000, "puff-duration": 500,
              "lightsout-duration": 5000, "max-corrections":10,
              "perch-delay": 1000, "prob_stim": 0.033,
...
function socialize() {
    var still_on_perch = true;
    var passed = false;
    var rand = 0;
    update_state({trial: state.trial, phase: "socializing"});
...
function puff() {
    ...
    //t.await("feeder_left", null, function(msg) { return msg.feeding == false }, _.partial(intertrial, par.min_iti));
    t.await("feeder_left", null, function(msg) { return msg.feeding == false }, await_init);
}
