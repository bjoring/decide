...
var apparatus = {
    ...
    hopper: [keys, { device: "/dev/input/event2", keymap: ["up"]}],
...
function req(msg, addr, data, rep) {
    ...
    else if (_.has(components, addr)) {
        components[addr].req(msg, data, rep);
    }
    else
        rep("component does not exist " + addr);
}

function register(key, klass, par) {
    components[key] = new (klass)(par, key, pub);
