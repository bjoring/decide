...
function connect() {
    ...
    socket.on("message", function(msg, payload) {
        if (msg == "OHAI-OK") {
        ...
        else if (msg == "WTF" || (msg == "RTFM" && !receiving)) {
            winston.error("server returned", msg);
            ...
            if (msg == "HUGZ") {
                ...
                var key = String(payload);
                if (_.has(queue, key))
                    winston.warn("server thinks %s is a duplicate", key)
            }
            else if (msg == "RTFM") {
                winston.warn("error processing message:", String(payload));
            }
        }
    })
    ...
    emitter.connect = function(addr) {
        ...
        socket.connect(addr);
        socket.send(["OHAI", "decide-host@1", os.hostname()]);
    };
