...
// convention of sending errors in the first argument)
...
// sends a request to change state
...
// returns an list of feeders in the system
function get_feeders(callback) {
    req("get-meta", {name: ""}, function(err, reply) {
        if (err) callback(err)
        else {
            callback(null,
                     _.map(reply, (val, key) => { return (val.type === "feeder") ? key : null})
...
function await(name, timeout, test, callback) {
    ...
    function _listen(msg) {
        logger.silly("await @%s got event", name, msg);
        if (!running) return;
    ...
    function _exit(time) {
        logger.silly("await @%s exiting at", name, time);
        running = false;
...
function heartbeat(name, data) {
    ...
    if (!host_params.heartbeat_interval) return;
