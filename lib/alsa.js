        ...
        if (msg.req == "change-state") {
            ...
            if (msg.data.playing) {
                ...
                aplay.stderr.on("data", function(data) {
                    if (/^Playing/.test(data) || /Rate/.test(data.toString()) || /bit/.test(data.toString())) {
                        state.playing = true;
                    }
                    else {
                        if (rep) rep(callback(data.toString().trimRight(), null, null, true));
                        else callback(data.toString().trimRight());
                    }
                });
