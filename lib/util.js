...
function StimSetPref(path) { //added a stimset for use in preference.js. 
    ...
    this.generate = function() {
    	this.stimuli = function() { //[peck_right, peck_left]
    		_.chain(this.config.choices)
    	}
    	for (var i in this.stimuli) {
    		this.stimuli[i] = _.chain(this.config.choices[i].stimuli) //brackets around [i]?
    			.map(function(stim) {return _.times(stim.frequency, _.constant(stim))}) //where did stim come from?
	    		.flatten()
    			.shuffle()
    			.value();
    	}//this.stimuli = [[A 5th, A 4th], [A 2nd, A 7th]]
    ...
    this.next = function(replace, pecked) {
		var choice = (pecked == peck_right)? 0 : 1; //this is not very flexible?
        var n = _.keys(this.stimuli.[choice]).length;
        if (replace) {
            var index = Math.floor(Math.random() * n);
        }
        else {
        ...
        return this.stimuli.[choice][index]
    }
