...
function slack(message, user, callback) {
    let msg = message;
    if (_.isArray(user)) {
        msg = format("%s: %s",
                     _.map(_.filter(user, _.isString), (n) => { return "<" + n + ">"; }).join(", "),
                     message);
    }
    else if (_.isString(user))
        msg = format("Hey <%s>: %s", user, message);
    request.post({url: config.slack_transport.hook, json: true, body: {text: msg}},
                 function(e, r, body) {
                     if (!callback) return;
                     if (e || (r && r.statusCode != 200))
                         callback(format("slack error: %s", body));
                     else
                         callback(null, "sent slack message");
                 });
}
