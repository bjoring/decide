...
function StimSet(path) {
    this.stimset = load_json(path);
    this.root = this.stimset.stimulus_root;

    function check_probs(resp) {
        var tot = resp.p_punish + resp.p_reward;
        console.log(resp + ": p(tot)=" + tot)
        if (tot > 1.0)
            throw "stimset response consequence probabilities must sum to <= 1.0";
    }

    // validate the probabilities in the stimset
    _.each(this.stimset.stimuli, function(stim) {
        _.each(stim.responses, check_probs)
    });

    this.generate = function() {
        this.stimuli = _.chain(this.stimset.stimuli)
