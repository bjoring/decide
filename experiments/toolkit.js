...
function toolkit() = {
	...
	this.initialize = function(name, object) {
		state.program = name;
		state.running = true;
		req.emit("route", "experiment", console.log);	
	}

	this.trial_loop = function(something) {
	...
	this.feed = function(which, duration, callback) { // TODO: change feed() to follow closure format
	...
	this.write_led = function(which, state) {
		req.emit("msg", {
			req: "change-state",
	...
	this.write_feed = function(which, state) {
		req.emit("msg", {
			req: "change-state",
			addr: "feeder_" + which,
			data: {
