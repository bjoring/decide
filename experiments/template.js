...
// create a training procedure.
	function example_cue() {										// blinks led until keypress
		...
		t.cue(default_cue).blink();									// blink the default led
		t.keys(default_key).wait_for(false, example_reward);		// run 'example_reward()' on default_key press
	}
	...
	function exaple_reward(data, feeder, cue) {						// feeds the subject, then signals end of trial
		...
		t.cue(default_cue).off();									// stop blinking led
		t.hopper(feed_select).feed(par.feed_duration, function() {	// feed the subject, then, once finished...
			trial_data.fed = true;									// update trial_data.fed
			end_trial();											// run 'end_trial()'
		});
	}
...
// handle all the clean up.
	function finished() {											// sinals end of training
		winston.info('Experiment Finished',Date.now());				// let others know training has finished
